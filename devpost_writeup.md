...
- **Real-time closed captions with no third-party STT.** The most invisible-looking feature turned out to be one of the hardest to implement correctly. The LiveKit ecosystem's standard caption solution requires Deepgram (a paid STT service) subscribing to the user's audio track. That works for student speech — but for *agent* speech, the Gemini 2.5 Live model already transcribes its own output word-by-word internally via `outputTranscription` chunks. These appear in the backend logs in real time (`{"serverContent":{"outputTranscription":{"text":" Today"}}}`), proving the data existed — but no public API or documentation described how to intercept it and forward it to the frontend. Digging into the compiled plugin source, we traced `outputTranscription` chunks into `gen.textChannel`, an internal `ReadableStream` created per response turn and never exposed publicly. The working approach: enable `outputAudioTranscription: {}` on `RealtimeModel` to guarantee the field is populated, then hook the `generation_created` event on the `RealtimeSession` (accessed via the private `_session` field with a type cast). For each generation, we read `textStream` word-by-word and call `localParticipant.publishTranscription()` — LiveKit's native data channel used by the `useTranscriptions()` React hook on the frontend. Each chunk is published as a non-final segment to update the display mid-sentence, then marked final when the turn ends. This delivers streaming word-level captions with zero cost, no third-party service, and no audio re-encoding — purely by intercepting the model's own self-transcription.
